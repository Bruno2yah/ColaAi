      ...
      <div class = "row h-100 linha">
        ...
        <div class="row align-items-center mb-4">
          <div class="col fs-3 fw-semibold">
            Lista de Avaliações Registrados
          </div>
          ...
          <table class="table table-hover">
            ...
            <?php foreach($avaliacao as $Avaliacao): ?>
             <tr> 
              ...
              <td><?=$Avaliacao['tituloAvaliacao'];?></td>
              <td><?=$Avaliacao['descAvaliacao'];?></td>
              <td><?=$Avaliacao['idOrganizacaoEvento'];?></td>
              <td class="text-center">
                  <form action="process.php" method="POST">
                    <input type="hidden" class="form-control" id="acao" name="acao" value="SELECTID">
                    <input type="hidden" class="form-control" id="id" name="id" value="<?=$Avaliacao['idAvaliacao']?>">
                    <button type="submit" class="dropdown-item"><i class="fas fa-edit fa-lg text-secondary"></i>
                    </button>
                  </form>
                </td>
                <td class="text-center">
                  <a class="dropdown-item" onclick="modalRemover(<?=$Avaliacao['idAvaliacao']?>,'idDeletar')">
